...
#    print(Total_NumSpan)
    ASE_PSD=2*h*nu/2*Total_NumSpan*(AmplifierNoiseFactor*AmplifierGain-1)
#    print(ASE_PSD)
    '''
    '''
...
'''
def NLSE_Solver_Link(inputSignal,freq_D,other_parameters):
...
if __name__=='__main__':
    ...
    PowerSet_dBm=arange(-5,11,1)           # Launch power vector (dBm)
    roll_off_factor=0.02                   # roll-off factor of Tx RRC filter
    ##### Span Parameters #####
    alpha=0.22/4.343*1e-3*1                # Fiber attenuation coefficient (1/m)
    beta2=-1.9378e-26*1                    # Fiber dispersion coefficient (s^2/m)
    gamma=1.3e-3*1                         # Fiber non-linear coefficient (1/(W.m))
    Lspan=140e3                            # Span fiber length (m)
    NF=5.5                                 # Span amplifier noise figure (dB)
    
    ##### Link Parameters ######
    Nspan=25                               # Number of spans per link
    
    ###### Transceiver parameters ############
    SymbolRate=32.5e9                                  # Symbol rate (Hz)
    ChannelBandwidth=SymbolRate*(1+roll_off_factor)    # Channel bandwidth (Hz)
